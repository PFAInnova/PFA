...
const Angular = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await axios.get('http://localhost:8000/api/courses/Angular');
  ...
  if (loading) return <div className="text-center text-gray-600">Chargement des vidéos...</div>;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-blue-100 to-purple-200">
      ...
      <div className="container mx-auto py-10 px-4">
        <h1 className="text-5xl font-bold text-indigo-600 mb-12 text-center">Bienvenue dans nos Cours Angular !</h1>
        <p className="text-xl text-gray-700 text-center mb-8">Découvrez nos vidéos et améliorez vos compétences en Angular à votre rythme. Chaque vidéo est conçue pour vous guider, de l'installation d'Angular à la création d'applications dynamiques et robustes.</p>
          ...
          {courses.map((course) => (
            ...
            >
              ...
              <p className="text-gray-600 mb-4">
                {course.description || "Dans ce cours, vous apprendrez les concepts clés d'Angular, comme les composants, les services, la gestion des routes, et la communication entre composants pour créer des applications interactives."}
              </p>
              ...
              <p className="text-indigo-500 text-sm text-center">Cliquez pour commencer à apprendre</p>
            </div>
          ))}
        </div>
      </div>
